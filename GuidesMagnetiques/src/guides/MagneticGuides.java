...
public class MagneticGuides extends JFrame {
	...
	private CExtensionalTag onGuideTag;
	...
	public MagneticGuides(String title, int width, int height) {
		...
		onGuideTag = new CExtensionalTag(canvas){};
		...
		CStateMachine sm = new CStateMachine() {
			...
			public State start = new State() {
				...
				Transition pressToGuideH = new Press(BUTTON1){
					public void action() {
						p = getPoint() ;
						MagneticGuide guide = new MagneticGuide(canvas, p, onGuideTag, true);
					}
				};
				
				Transition pressToGuideV = new Press(BUTTON3){
					public void action() {
						p = getPoint() ;
						MagneticGuide guide = new MagneticGuide(canvas, p, onGuideTag, false);
					}
				};
				
				Transition pressOnGuide = new PressOnTag(onGuideTag, BUTTON1, ">> oDrag"){
					public void action() {
						p = getPoint();
						draggedShape = getShape();
					}
